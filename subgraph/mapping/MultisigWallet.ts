...
export function handleRevokeDelegation(event: RevokeDelegation): void {
    let signer = Signer.load(event.address.toHex() + event.params.signer.toHex());
    if (signer) {
...
export function handleSignerAdded(event: SignerAdded): void {
    let signer = Signer.load(event.address.toHex() + event.params.signerAddress.toHex());
    if (!signer) {
        signer = new Signer(event.address.toHex() + event.params.signerAddress.toHex());
        ...
        if (ipfsData) {
            ...
            if (object) {
                let signerMetadata = new SignerMetadata(event.address.toHex() + event.params.signerAddress.toHex()
                );
                ...
                signerMetadata.save();
                signer.metadata = event.address.toHex() + event.params.signerAddress.toHex();
            }
        }
        signer.save();
        ...
        if (wallet) {
            ...
            tempSigners.push(event.address.toHex() + event.params.signerAddress.toHex());
...
export function handleERC20TransactionApproved(
    ...
    let txn = ERC20Transaction.load(event.params.txnId.toString());
    let approver = Signer.load(event.address.toHex() + event.params.approver.toHex());
    if (txn && approver) {
        txn.approval = txn.approval.plus(approver.weight);
        let tempApprovedBy = txn.approvedBy;
        if (tempApprovedBy) {
            tempApprovedBy.push(event.address.toHex() + event.params.approver.toHex());
...
export function handleERC20TransactionDisapproved(
    ...
    let txn = ERC20Transaction.load(event.params.txnId.toString());
    let disapprover = Signer.load(event.address.toHex() + event.params.disapprover.toHex());
    if (txn && disapprover) {
        txn.disapproval = txn.disapproval.plus(disapprover.weight);
        let tempDisapprovedBy = txn.disapprovedBy;
        if (tempDisapprovedBy) {
            tempDisapprovedBy.push(event.address.toHex() + event.params.disapprover.toHex());
